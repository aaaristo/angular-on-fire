  ...
  return {
    ...
    link: function(scope, element, attrs) {
      element.bind('click', function(evt){    
        ...
        var loader= scope.$eval(attrs.loader);

        if (loader)
        loader(function ()
        {
           _.defer(function () { PrintElem(attrs.printDiv); });
        });
      });
      ...
      function PrintWithIframe(data) 
      {
        if ($('iframe#printf').size() == 0) {
          $('html').append('<iframe id="printf" name="printf" style="visibility: hidden;"></iframe>');  // an iFrame is added to the html content, then your div's contents are added to it and the iFrame's content is printed
